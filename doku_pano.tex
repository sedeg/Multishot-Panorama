...
\begin{document}
	...
	\chapter{Einleitung}
	\label{ch:einleitung}
	Im Rahmen des \textit{Moduls Multishot-Technik in der digitalen Fotografie} wurde diese Dokumentation zu dem Thema \textit{Panoramafotografie} realisiert. Dafür werden mehrere 180$^\circ$- und 360$^\circ$- Panoramen, sowie ein Kugelpanorama umgesetzt. Damit ein Panorama aus mehren Einzelaufnahmen zusammengesetzt werden kann, muss man einige Besonderheiten beachten, da es sonst zu Fehlern kommt. Dies betrifft sowohl, die Vorbereitung der Technik, wie beispielsweise die Justage des No Parallax Point, sowie die Nachbereitung, also das sogenannte Stitching der Einzelaufnahmen zu einem Panorama. 
	
	\bigskip
	Im Verlauf dieser Arbeit wird detailliert gezeigt, wie die insgesamt sechs Panoramen erstellt werden. Dabei wird die richtige Kalibrierung der Technik, die jeweilige Bildidee, das Stitching, bis hin zur Nachbearbeitung genau erläutert.
		
	\chapter{Justage des NPP}
	\label{sec:npp}
	Um das Auftreten von Parallax-Fehlern im Vordergrund bei der Zusammensetzung der Bildreihe zu vermeiden, gilt es vor der Aufnahme den No Parallax Point (NPP) einzustellen. Das bedeutet, dass die Kamera um die Eintrittspupille des Objektivs zu drehen ist. Die beiden Nodalpunkte des Systems befinden sich vor und hinter diesem Punkt. Deshalb gibt es für die Panoramafotografie spezielle Nodalpunktadapter, durch welche eine präzise Ausrichtung, mittels einer horizontalen und einer waagerechten Schiene, des Kamerasystems möglich ist.

	\bigskip
	Justiert wird durch das Verschieben der Kamera mittels zweier Schlitten. Durch den horizontal gelegenen erfolgt die Ausrichtung des Objektives zentral über dem Stativkopf. Die zweite in waagerechter Haltung befindliche Schiene kann nach vorn und hinten verschoben werden. Dadurch soll erreicht werden, dass sich Objekte im Vordergrund bei einem Kameraschwenk, im Bezug auf ein Hintergrundobjekt, nicht verschieben. In der Abbildung \ref{fig:justage} ist ein solcher Kameraschwenk zu sehen. Dabei befindet sich die Laterne im Vordergrund und die Garageneinfahrt im Hintergrund. Aus den Aufnahmen geht deutlich hervor, dass die Laterne ihre Position bei einem Schwenk nicht verändert und der NPP somit korrekt eingestellt ist. 
	...
	Charakteristisch für die Leipziger Innenstadt, ist die Vielzahl an Passagen und Durchgangshöfen, welche das Stadtbild prägen. Die älteste, erhaltene Ladenpassage ist Speck\grq s Hof, welche zwischen 1908 und 1929, durch den Bau des Messehauses entstand. Den Namen erhielt die Passage auf Grund des Kaufhofes des Freiherrn von Speck, welches zuvor an dieser Stelle stand. Sie befindet sich an der Kreuzung Reichstraße / Grimmaische Straße, in direkter Nähe zur Nikolaikirche und ist mit dem Hansa Haus verbunden. In den Jahren 1993 bis 1995 wurde die Passage aufwendig restauriert. Heute bietet Speck\grq s Hof eine architektonische und gestalterische Mischung aus Vergangenheit und Moderne und ist beispielsweise durch Malereien und Plastiken der Künstler Bruno Griesel, Johannes Grützke und Moritz Götze verziert. Einen besonders markanten Punkt bildet dabei der Klangbrunnen, welcher sich zentral in dem acht-eckigen Lichthof der Passage befindet.
		...
		\begin{minipage}{0.58\textwidth}
			\begin{tabular}{ll}
				...
				& F/3.5-4.5 USM\\		
				Brennweite:& 10mm \\
				Belichtungszeit: & $\frac{1}{2}$s - 1.3s\\
				Blendenwert: & f/8\\
	...
	\subsubsection{Vorgehen und Fehleranalyse}
	 Die Aufnahmereihe entstand am 05.11.2016 um 16:58 Uhr. Dabei wurde die Kamera nahe dem Eingang platziert und mit einem vertikalen Winkel von 0$^\circ$ mittig auf den Klangbrunnen ausgerichtet.  Zunächst wurden die Einzelbilder mit einem horizontalen Versatz von 45$^\circ$ aufgenommen. Während des späteren Stitchings stellte sich heraus, dass diese Überlappung der einzelnen Bilder auf Grund der sehr geringen Brennweite von 10mm und der starken Objektivverzerrung am Bildrand, nicht ausreichend ist, um die Bilder fehlerfrei zusammenzusetzen (siehe Abbildung \ref{img:error}). Um dies zu beheben wurden erneut sieben Teilbilder, mit einem horizontalen Versatz von 30$^\circ$ aufgenommen und diese später, durch das in Punkt \ref{ch:stitiching} beschriebene Stitching, ohne Fehler zu einem 180$^\circ$ Panorama zusammengefügt. Auf Grund der zuvor durchgeführten Justage des NPP, sind keine Parallaxverschiebungen aufgetreten.
	 
	 \begin{figure}[H]
	 	\includegraphics[width=\linewidth]{img/error.jpg}
	 	\caption{Fehlerhaftes Stitching auf Grund eines zu hohen horizontalen Versatzes}
	 	\label{img:error}
	 \end{figure}
	...
	\subsubsection{Vorgehen und Fehleranalyse}
	Für die Aufnahme der Einzelbilder wurde genauso vorgegangen, wie es bereits unter Punkt \ref{sec:specks} beschrieben ist. Die Kamera wurde mit einem vertikalen Winkel von 0$^\circ$ mittig zwischen den beiden Statuen positioniert. Diese befinden sich am rechten und linken Rand des 180$^\circ$ Panoramas. Durch einen horizontalen Versatz von 30$^\circ$ entstanden sieben Teilbilder, welche am 05.11.2016 um 17:07 Uhr aufgenommen wurden. 

	 \newpage
	 \begin{figure}[h]
	 	\includegraphics[width=\linewidth]{img/ph.jpg}
	 	\caption{180$^\circ$ Panorama -- Auerbachskeller}
	 \end{figure}
	...
	\bigskip
	Der Platz wird eingerahmt von dem Geschäftshaus \grqq{}Großer Blumenberg\grqq{}, den Höfen am Brühl, dem Goerdelerring und der Evangelisch Reformierte Kirche zu Leipzig. Besonders die Fassade der ehemaligen \grqq{}Blechbüchse\grqq{} ist sehr markant und bildet zusammen mit den vier Springbrunnen, welche sich direkt auf dem Platz befinden einen modernen Gegensatz zur restlichen Altstadt Leipzigs.
	
	\bigskip
	Zahlreiche Geschäfte, Bars und Restaurants sind am Abend beleuchtet und tauchen den Platz in ein angenehmes Licht, mit einer ruhigen und warmen Atmosphäre. Sodass dieser für das 180$^\circ$ Nachtpanorama gewählt wurde.
	...
	\subsubsection{Vorgehen und Fehleranalyse}
	Die vier Teilaufnahmen entstanden am 02.11.2016 um 18:26 Uhr. Zu dieser Zeit war es bereits so dunkel, dass eine entsprechend lange Belichtungszeit von fünf bis acht Sekunden notwendig war. Auf Grund dessen musste darauf geachtet werden, dass sich keine Fahrräder beziehungsweise Autos während der Belichtung durch das Bild bewegen, da deren Licht ansonsten zu dem sogenannten \grqq{}Lightstream-Effekt\grqq{} führen (siehe Abbildung \ref{img:ls}). Die lange Belichtungszeit führt dazu, dass sich bewegende Objekte, wie Personen, verschwinden und auf den Einzelbildern kaum vorhanden sind. Starre Elemente hingegen, wie beispielsweise der Springbrunnen im Vorder- und die Gebäude im Hintergrund sind klar erkennbar. 
		...
		\end{figure}
			 ...
			 \begin{figure}[h]
			 	\includegraphics[width=\linewidth]{img/ph.jpg}
			 	\caption{180$^\circ$ Nachtpanorama -- Richard Wagner Platz}
			 \end{figure}
	...
	Am östlichen Teil des Leipziger Innenstadtrings befindet sich der Augustusplatz, welcher von repräsentativen und kulturellen Bauwerken umgeben ist. Die beiden bekanntesten sind das Gewandhaus und die Leipziger Oper, welche sich frontal gegenüberstehen. Unmittelbar vor dem Gewandhaus steht der Mendebrunnen, welcher um 1880 entstand und als letztes historisches Bauwerk dieses Platzes, sowohl Angriffe des zweiten Weltkrieges, als auch Baumaßnahmen der DDR-Zeit überstand. Daneben finden sich unter anderen das City Hochhaus, das neue Augusteum und Paulinum der Universität Leipzig, das Krochhochhaus und die alte Handelspost. Diese prägen das Stadtbild Leipzigs und sorgen dafür, dass der Augustusplatz ideal für ein 360$^\circ$ Panorama geeignet ist.
		...
		\begin{minipage}{0.58\textwidth}
			\begin{tabular}{ll}
				...
				& F/3.5-4.5 USM\\		
				Brennweite:& 10mm \\
				Belichtungszeit: &$\frac{1}{40}$s - $\frac{1}{400}$s \\
				Blendenwert: & f/8\\
				Empfindlichkeit & ISO 100 \\
			\end{tabular}\\
		\end{minipage}%
	...
	\bigskip
		Aufgrund des sehr wechselhaften Wetters mussten die 13 Teilbilder zügig hintereinander aufgenommen werden. Wobei auf Grund des Wechsels von Sonne und Wolken immer wieder die Belichtungszeit angepasst werden musste. Durch die genaue Justage des No Parallax Point, treten im späteren 360$^\circ$ Panorama keine Parallax-Effekte auf. Das Stitching konnte problemlos durchgeführt werden, ohne dass falsche Überlappungen oder Verschiebungen auftreten, was besonders gut an dem kleinteiligen Boden erkennbar ist.

			 \newpage
			 \begin{figure}[h]
			 	\includegraphics[width=\linewidth]{img/ph.jpg}
			 	\caption{360$^\circ$ Panorama -- Augustusplatz}
			 \end{figure}
	...
	\label{sec:markt}
		...
		\subsubsection{Vorgehen und Fehleranalyse}
		Am 02.11.2016 um 18:14 Uhr wurden dreizehn Einzelaufnahmen gemacht, um dieses 360$^\circ$ Nachtpanorama zu realisieren. Dabei wurde auf den richtigen Zeitpunkt gewartet, damit die Bilder während der Blauen Stunde aufgenommen werden. Die Blaue Stunde bezeichnet dabei die Zeit während des Sonnenuntergangs und der Dunkelheit, in welcher der Himmel in ein tiefes Blau getaucht ist und in etwa die gleiche Helligkeit besitzt wie das  künstliches Licht der umgebenen Gebäude. Es wurden mehrere Testaufnahmen gemacht, um den perfekten Zeitpunkt abzupassen (siehe Abbildung \ref{img:bs}).
		...
		\end{figure}
		 ...
		 \begin{figure}[h]
		 	\includegraphics[width=\linewidth]{img/ph.jpg}
		 	\caption{360$^\circ$ Nachtpanorama -- Marktplatz}
		 \end{figure}

	\section{Kugelpanorama --  Mädler-Passage}
	\label{sec:kugel}
			\subsubsection{Aufnahmeort und -idee}
			Wie im Abschnitt \ref{sec:specks} bereits erläutert, ist das Bild der Leipziger Innenstadt von zahlreichen Passagen geprägt. Eine der bekanntesten, größten und vollständig erhaltenen ist die Mädler-Passage in welcher sich auch der Auerbachskeller befindet. Den Namen verdankt die Passage Anton Mädler, welcher sämtliche umliegenden Gebäude abreißen ließ. Anschließend beauftragte er den Bau das Messehauses Mädler-Passage, welcher 1912 begann und 1914 abgeschlossen wurde. Die Mädler-Passage besteht aus drei Gängen, welche den Neumarkt, den Messehof und die Grimmaische Straße miteinander verbinden und sich in einer Zentralen Rotunde vereinen. Diese bietet einen Rundumblick in jeden der Gänge und eignet sich auf Grund der historischen Architektur hervorragend für die Realisierung eines Kugelpanoramas.
			\begin{figure}[H]
			...
			\begin{minipage}{0.42\textwidth}
				\begin{figure}[H]
					...
					\caption{Aufnahmesituation innerhalb der Mädler-Passage}
					\label{img:ak}
				\end{figure}
			\end{minipage}%
			...
			\bigskip
			Es wurde versucht möglichst wenig Personen mit abzubilden, aber auf Grund der hohen Frequentierung der Passage konnte dies nicht vollständig vermieden werden. Allerdings ist dies, auf Grund der Bildidee einen Einblick in das Treiben innerhalb der Passage zu zeigen, nicht problematisch. Während des Stitchings treten keine Parallax-Fehler auf, was auf die richtige Justage des NPP zurückzuführen ist. 
			
					 \newpage
					 \begin{figure}[h]
					 	\includegraphics[width=\linewidth]{img/ph.jpg}
					 	\caption{Kugelpanorama --  Mädler-Passage}
					 \end{figure}
			
	\chapter{Stitching}
	\label{ch:stitiching}
	Die für ein Panorama aufgenommenen Einzelbilder werden in der digitalen Fotografie, mittels des sogenannten Stitchings, zu einem Gesamtbild zusammengesetzt. Um diesen Vorgang weitestgehend zu automatisieren, gibt es verschiedene kostenlose oder auch kostenpflichtige Programme. In dieser Arbeit wird dabei näher auf Photoshop (\ref{sec:photoshop}) und PTGui Pro (\ref{sec:ptgui}) eingegangen, da diese für die Erstellung und Bearbeitung im Vordergrund stehen. Eine kostenfreie Softwarealternative für das Stitchen ist Hugin, welches in dieser Arbeit allerdings keine Verwendung findet, da die eben genannten Alternativen bereits alle benötigten Funktionen beherrschen.
	...
	\end{figure}
	\bigskip
	Mit einem Klick auf \grqq{}Align Images\grqq{} beginnt die Software mit dem Stitching der Einzelbilder. Anschließend öffnet sich ein weiteres Fenster, auf welchem sich dass zusammengefügte Panorama betrachten lässt. Zudem werden den bereits bestehenden Tabs weitere angefügt s. Abb. \ref{img:ptgui_step_4}. 
	\begin{figure}[H]
	...
	\end{figure}
	\bigskip
	Wie in Abb. \ref{img:ptgui_step_4_1} zu sehen ist, kann man mit Hilfe der Maskierung in PTGui Pro, Bereiche kennzeichnen. Diese geben an, ob bestimmte Bereiche des Fotos im fertigen Panorama vorhanden (grün) oder nicht  vorhanden (rot) sein sollen. Die Software versucht diesem Wunsch nachzukommen und nimmt für rot markierte Bereiche überlappende Teile aus dem Nachbarbild und fügt diese stattdessen ein.
	\begin{figure}[H]
	...
	\end{figure}
	\bigskip
	Zudem bietet die Software die Möglichkeit für die jeweils zusammengefügten Fotos die sog. Kontrollpunkte zu überprüfen, zu verändern oder neue zu setzen. Mit Hilfe dieser Kontrollpunkte können in benachbarten Bildern Referenzpunkte festgelegt werden, sodass die Software erkennt, an welchen Stellen die beiden Bilder zusammengefügt werden müssen. Dies ist in Abb. \ref{img:ptgui_step_5} zu sehen. Ein Kontrollpunkt auf dem linken Foto muss dabei einen auf dem rechten Bild entsprechen.
	\begin{figure}[H]
	...
	\label{sec:photoshop}
	Adobe Photoshop beinhaltet das Tool \grqq{}Photomerge\grqq{}, mit dessen Hilfe Bilderreihen zu Panoramen zusammengefügt werden können. Dieses Kapitel befasst sich deswegen mit dem Stitching mittels Photoshop, wobei zugleich das Programm PTGui Pro (Kap. \ref{sec:ptgui}) als Vergleich herangezogen wird.
	\bigskip
